...
async function requestMultipart(baseUrl, path, { token = null, formData } = {}) {
  const response = await fetch(`${baseUrl}${path}`, {
    method: 'POST',
    headers: {
      ...(token ? { Authorization: `Bearer ${token}` } : {})
...
export function createRoomRentalClient({ baseUrl = resolveApiBaseUrl(), token = null } = {}) {
  return {
    ...
    fetchListingById: (listingId) => requestJson(baseUrl, `/listings/${listingId}`),
    fetchMyListings: (params = {}) => requestJson(baseUrl, `/listings/me${buildQuery(params)}`, { token }),
    ...
    addListingVideo: (listingId, payload = {}) => {
      if (payload.videoFile) {
        ...
        return requestMultipart(baseUrl, `/listings/${listingId}/videos`, { token, formData });
      }

      return requestJson(baseUrl, `/listings/${listingId}/videos`, {
        method: 'POST',
        token,
        body: payload
      });
    ...
    fetchLandlordInquiries: (params = {}) => requestJson(baseUrl, `/engagement/inquiries/received${buildQuery(params)}`, { token }),
    updateLandlordInquiryStatus: (inquiryId, payload) => requestJson(baseUrl, `/engagement/inquiries/${inquiryId}/status`, {
      method: 'PATCH',
      token,
      body: payload
    }),
    ...
    requestImmersiveGeneration: (listingId, payload = {}) => requestJson(baseUrl, `/immersive/listings/${listingId}/generate`, {
      method: 'POST',
      token,
      body: payload
    }),
...
export { buildQuery, requestJson, requestMultipart };
